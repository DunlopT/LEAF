...
This script comprises the main code for the LEAF model version 2.0 (dated 20.06.2025) as outlined in the following publication:
Dunlop T., Felder, S., Glamore, W. 2025. Mangrove restoration is guided by lifecycle responses to climatic forcing.

The original LEAF model version 1.0 (dated 31.01.2025) is outlined in the following publication:
...
For simplicity in LEAF version 2.0, all lifecycle calculations are conducted in this code.


v.2.0 Updates (20.06.2025)

Pneumatophore growth
- A new input parameter (pnelimit) has been included for the pneumatophore growth ceiling relative to the local bed level. This is typically set to 56% of the tidal range (Dunlop et al., 2025).
- Pneumatophores will continue to grow as long as they have not reached this ceiling.
- If the minimum water level is above the pneumatophore height for consecutive timesteps, pneumatophores will continue to grow (i.e., during floods).
Failure mechanisms from waves and winds
- Seedling dislodgement
    - Seedlings will dislodge if the wave force is greater than the dislodgement threshold, calculated based on stem height in accordance with data from Kibler et al., 2022.
- Overturning
    - Saplings/Adults will overturn if the wind or wave moment is greater than the threshold moment for overturning
- Breaking
    - Saplings/Adults will break if the wind or wave moment is greater than the threshold moment for breaking
Growth rates
- In general, growth rates have been updated so that mangrove attributes are not directly related to another attribute, but instead their growth rate.
- The allometric relationships between stem height and diameter based on field studies from Dunlop et al. (2025) and Trettin et al. (2016) have been included.
...
input_file = os.path.join(workdir,'dflowfm','LEAF_inputs_v.2.0.json') # User defined inputs file
...
# Plotting parameters
xsecycoords = [12.5, 37.5] # Update y-coordinates of the cross-sections that are plotted
...
inipneums = parameters['3. Initial mangroves']['inipneums'] # 3F. Pneumatophore presence (Y/N) (v.2.0 update)
inipneelev = parameters['3. Initial mangroves']['inipneelev'] # 3G. Pneumatophore elevation (m) (v.2.0 update)
inipnedia = parameters['3. Initial mangroves']['inipnedia'] # 3H. Pneumatophore diameter (m)
iniage = parameters['3. Initial mangroves']['iniage'] # 3I. Age (years)
# 4. Mangrove extrema
...
pnelimit = parameters['5. Other mangrove parameters']['pnelimit'] # 5I. Pneumatophore ceiling based on tidal range. Elevation relative to local datum (i.e., mAHD) (v.2.0 update)
meanwooddensity = parameters['5. Other mangrove parameters']['meanwooddensity'] # 5J. Mean wood density (kg/m3) (v.2.0 update)
# 6. Establishment
...
# 8G. Seedling dislodgement (v.2.0 update)
dislodgeactive = parameters['8. Recovery and mortality']['dislodgeactive'] # 8G1. Seedling dislodgement mortality activation (Y/N)
waveforce = parameters['8. Recovery and mortality']['waveforce'] # 8G2. Wave force (N or formula)
dislodgethresh = parameters['8. Recovery and mortality']['dislodgethresh'] # 8G3. Dislodgement threshold (N or formula)
# 8H. Overturning (v.2.0 update)
overturnactive = parameters['8. Recovery and mortality']['overturnactive'] # 8H1. Overturning mortality activation (Y/N)
windforce = parameters['8. Recovery and mortality']['windforce'] # 8H2. Wind force (N or formula)
overturnthresh = parameters['8. Recovery and mortality']['overturnthresh'] # 8H3. Critical overturning moment (Nm or formula)
dragcoeffwind = parameters['8. Recovery and mortality']['dragcoeffwind'] # 8H4. Wind drag coefficient (Nm or formula)
overturnbiomass = parameters['8. Recovery and mortality']['overturnbiomass'] # 8H5. Biomass equations for overturning (-)
# 8I. Breaking (v.2.0 update)
breakactive = parameters['8. Recovery and mortality']['breakactive'] # 8I1. Breaking mortality activation (Y/N)
breakingthresh = parameters['8. Recovery and mortality']['breakingthresh'] # 8I2. Critical breaking moment (Nm or formula)
# 9. Functionality
...
dmgregdis = np.zeros((maxcapacity, ndx)) # Damage regime tracker for seedling dislodgement (v.2.0 update)
dmgregover = np.zeros((maxcapacity, ndx)) # Damage regime tracker for overturning (v.2.0 update)
dmgregbrk = np.zeros((maxcapacity, ndx)) # Damage regime tracker for breaking (v.2.0 update)
...
dmgregdisgc = np.zeros(ndx) # Damage regime tracker for seedling dislodgement in each grid cell (v.2.0 update)
dmgregovergc = np.zeros(ndx) # Damage regime tracker for overturning in each grid cell (v.2.0 update)
dmgregbrkgc = np.zeros(ndx) # Damage regime tracker for breaking in each grid cell (v.2.0 update)
        ...
        for b in range(inistemsgrid): # For every initial stem
                ...
                if floor(vegtrkrt[b,j]/pneumint) >= 1: # If the cable root reaches next interval and tracker matrix has no pneumatophore
                    for r in range(floor(vegtrkrt[b,j]/pneumint)): # For all intervals up to the length of the cable root that pneumatophores could exist
                        # Check to see whether existing pneumatophores exist, and assign properties accordingly (v.2.0 update)
                        if inipneums == "Y": # If the existing mangroves have pneumatophores
                            if inipneelev - bedlevel[j] + cablertdepth > 0: # If the initial pneumatophore length is positive
                                ...
                                vegtrkpnelen[b,j,r] = inipneelev - bedlevel[j] + cablertdepth # Give an initial length to the pneumatophore at timestep 0, equal to the set elevation minus the bedlevel plus the cable root depth
                                vegtrkpnedia[b,j,r] = inipnedia # Give an initial base diameter to the pneumatophore
                        elif inipneums == "N": # If the existing mangroves do not have pneumatophores
                            vegtrkpnelen[b,j,r] = 0 # Assign pneumatophore length to 0
                            vegtrkpnedia[b,j,r] = 0 # Assign pneumatophore diameter to 0
...
for p in range(ndx): # Minor change to how the range is written (v.2.0 update)
...
for i in range(0,nt): # Where i represents the time step
    ...
    maxvel = is_maxvalsnd[range(ndx),1] # Create an array with max velocity (for the timestep) for each non-boundary grid cell (v.2.0 update)
    ...
    # Retrieve wave and wind parameters (v.2.0 update)
    # Waves
    hwav = model_dfm.get_var('hwav') # Root mean squared wave height, Hrms
    if hwav is not None:
        hm0 = hwav * math.sqrt(2) # Significant wave height at the toe, Hm0
    # Wind
    wx = model_dfm.get_var('wx')
    ...
    for j in range(ndx): # Where j represents the grid cell
        ...
        if j < len(x_coord): # Limiting the calculation to non-boundary cells
            # Check to ensure fecund adults haven't suffered mortality (v.2.0 update)
            if any((vegtrkage[b,j] >= fecundity * 365 * 24 * 3600 / mtpervt) and (dmgregall[b,j] !=2) for b in range(maxcapacity)): # If the mangrove age in timesteps is greater than fecundity age in timesteps
                vegtrkadult[j] = 1 # Set fecund mangrove (adult) tracker to 1 for the grid cell
            else:
                vegtrkadult[j] = 0 # Set fecund mangrove (adult) tracker to 0 if there are no fecund adults or all adults have suffered mortality
        ...
        if mortactive == 'Y': # Check if user has activated the recovery and mortality stage
            for b in range(maxcapacity): # For every possible stem
                if vegtrkht[b,j] != 0: # For every non-zero value in the stem height array
                    ...
                    if vegtrkht[b,j] < sapht: # If the mangrove stem is a seedling (based on height)
                        ...
                        if inundactive == 'Y': # Check if user has activated inundation
                            if dmgregin[b,j] != 2: # If the damage regime for the mangrove stem is not B (Mortality), then calculate updates to damage regime
                                ...
                                if c_instr[b,j] == inundtimeseed2: # If the counter reaches the threshold for the inundation to cause mortality
                                    ...
                                    c_inrec[b,j] = 0 # Reset recovery counter (v.2.0 update)
                                    c_instr[b,j] = 0 # Reset counter for being impacted by inundation (v.2.0 update)
                        ...
                        if desiccactive == 'Y': # Check if user has activated desiccation
                            if dmgregde[b,j] != 2: # If the damage regime for the mangrove stem is not B (Mortality), then calculate updates to damage regime
                                ...
                                if c_destr[b,j] == desicctimeseed2: # If the counter reaches the threshold for the desiccation to cause mortality
                                    ...
                                    c_derec[b,j] = 0 # Reset recovery counter (v.2.0 update)
                                    c_destr[b,j] = 0 # Reset counter for being impacted by desiccation (v.2.0 update)
                        ...
                        # 4. Overturning # (v.2.0 update)
                        if overturnactive == "Y":
                            # Overturning of stems occurs when the wind or wave force exceeds the critical overturning moment
                            if dmgregover[b,j] != 2: # If the damage regime for the mangrove stem is not B (Mortality), then calculate updates to damage regime
                                # Calculate the wave force
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for wave force calculations
                                if hwav is not None and waveforce == "morison1950":
                                    waterdensity = 1025 # Seawater density (kg/m3)
                                    wavecelerity = math.sqrt(9.81 * hm0[j]) # Speed of wave in shallow water (m/s)
                                    cd_morison = 1.2 # Assumed drag coefficient for thin vertical stem
                                    fwave = 0.5 * waterdensity * cd_morison * vegtrkdia[b,j] * wavecelerity ** 2 # (N)
                                if isinstance(waveforce, (int, float)):
                                    fwave = waveforce # (N)
                                # Calculate the wind force
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for wind force calculations
                                if wx is not None and windforce == "quaddrag":
                                    # Air density
                                    airdensity = 1.2 # kg/m3
                                    # Drag coefficient
                                    # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for drag coefficient
                                    if dragcoeffwind == "rudnicki2004":
                                        cdwind = 1.0 # Cd varies between 0.2 and 1 (Rudnicki et al., 2004)
                                    ...
                                    if projareastem == "cylinder":
                                        stemarea = vegtrkdia[b,j] * vegtrkht[b,j]
                                    fwind = 0.5 * airdensity * cdwind * stemarea * wx[j] ** 2 # Quadratic drag equation (N)
                                if isinstance(windforce, (int, float)):
                                    fwind = windforce # N
                                # Calculate wind moment
                                if wx is not None:
                                    windmoment = fwind * vegtrkht[b,j] # Nm
                                # Calculate wave moment
                                if 'fwave' in locals():
                                    wavemoment = fwave * min(maxwd[j], vegtrkht[b,j]) # Moment calculated at the water level for simplicity (Nm)                            
                                # Calculate critical overturning moment
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for critical overturning moment
                                if overturnthresh == "nicoll2006":
                                    # Constants
                                    cr = 60 # Dimensional regression constant. This varies from 60-200m2/s2 (Table 5 of Nicoll et al., 2006)
                                    # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for biomass used in overturning moment calculation
                                    if overturnbiomass == "komiyama2005": # Weight equations for Avicennia alba from felled trees
                                        ws = 0.0696 * meanwooddensity * (vegtrkdia[b,j] ** 2 * vegtrkht[b,j]) ** 0.931
                                        wl = 0.135 * meanwooddensity * vegtrkdia[b,j] ** 1.696
                                        wtop = 0.251 * meanwooddensity * vegtrkdia[b,j] ** 2.46
                                        wr = 0.199 * meanwooddensity ** 0.899 * vegtrkdia[b,j] ** 2.22
                                        wtotal = ws + wl + wtop + wr
                                    critovermoment = cr * wtotal # Nm (Nicoll et al., 2006; Quine and Gardiner, 2007)
                                if isinstance(overturnthresh, (int, float)):
                                    critovermoment = overturnthresh
                                if wx is not None and windmoment >= critovermoment: # If the wind moment >= critical overturning moment
                                    dmgregover[b,j] = 2 # Set damage tracker to 2 (mortality) so that we can output which mangroves are affected by overturning
                                    for item in vegtrklist: # Set all veg properties to zero
                                        item[b,j] = 0 
                                    vegtrkht[b,j] = 0
                                    vegtrkpnechk[b,j,:] = 0
                                    vegtrkpnelen[b,j,:] = 0
                                    agpnelen[b,j,:] = 0
                                    vegtrkpnedia[b,j,:] = 0
                                if 'wavemoment' in locals() and wavemoment >= critovermoment: # If the wave moment >= critical overturning moment
                                    dmgregover[b,j] = 2 # Set damage tracker to 2 (mortality) so that we can output which mangroves are affected by overturning
                                    for item in vegtrklist: # Set all veg properties to zero
                                        item[b,j] = 0 
                                    vegtrkht[b,j] = 0
                        ...
                        # 5. Breaking # (v.2.0 update)
                        if breakactive == "Y":
                            # Breaking of stems occurs when the wind or wave force exceeds the critical breaking moment
                            if dmgregbrk[b,j] != 2: # If the damage regime for the mangrove stem is not B (Mortality), then calculate updates to damage regime
                                # Calculate the wave force
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for wave force calculations
                                if hwav is not None and waveforce == "morison1950":
                                    waterdensity = 1025 # Seawater density (kg/m3)
                                ...
                                if 'fwave' in locals():
                                    wavemoment = fwave * min(maxwd[j], vegtrkht[b,j]) # Moment calculated at the water level for simplicity (Nm)
                                # Calculate critical breaking moment
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for critical breaking moment
                                if breakingthresh == "quine2007":
                                    # Constants
                                    fknot = 0.8 # Knot factor varies from 0.8-1 (-) (Quine and Gardiner, 2007)
                                    modrupture = 32 # Mean modulus of rupture, N/mm2 (van Hespen et al., 2021)
                                    modrupture = modrupture * 1000000 # Conversion to N/m2
                                    critbrkmoment = math.pi/32 * fknot * modrupture * (vegtrkdia[b,j]) ** 3 # Nm (Quine and Gardiner, 2007)
                                if isinstance(breakingthresh, (int, float)):
                                    critbrkmoment = breakingthresh
                                if wx is not None and windmoment >= critbrkmoment: # If the wind moment >= critical breaking moment
                                    dmgregbrk[b,j] = 2 # Set damage tracker to 2 (mortality) so that we can output which mangroves are affected by overturning
                                    for item in vegtrklist: # Set all veg properties to zero
                                        item[b,j] = 0 
                                    vegtrkht[b,j] = 0
                                    vegtrkpnechk[b,j,:] = 0
                                    ...
                                    agpnelen[b,j,:] = 0
                                    vegtrkpnedia[b,j,:] = 0
                                if 'wavemoment' in locals() and wavemoment >= critbrkmoment: # If the wave moment >= critical breaking moment
                                    dmgregbrk[b,j] = 2 # Set damage tracker to 2 (mortality) so that we can output which mangroves are affected by overturning
                                    for item in vegtrklist: # Set all veg properties to zero
                                        item[b,j] = 0 
                                    vegtrkht[b,j] = 0
                                    ...
                                    agpnelen[b,j,:] = 0
                                    vegtrkpnedia[b,j,:] = 0
                                # Reset wave and wind moments, and wave force
                                fwave = 0
                                wavemoment = 0
                                windmoment = 0

                        # 6. Dislodgement # (v.2.0 update)
                        # Dislodgement of seedlings occurs when the wave force exceeds a limit based on the height of the stem
                        if dislodgeactive == "Y": # Check if user has activated seedling dislodgement
                            if vegtrkht[b,j] > 0.05 and dmgregdis[b,j] != 2: # If the stem is greater than 5cm and damage regime is not B (Mortality)
                                # Calculate the wave force
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for wave force calculations
                                if hwav is not None and waveforce == "morison1950":
                                    waterdensity = 1025 # Seawater density (kg/m3)
                                ...
                                if isinstance(waveforce, (int, float)):
                                    fwave = waveforce # (N)
                                # Calculate dislodgement threshold
                                # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for seedling dislodgement threshold
                                if dislodgethresh == "kibler2022":
                                    fdislodge = 1.7724 * vegtrkht[b,j] * 100 - 4.7618 # Dislodgement threshold (N). Relationship derived from Avicennia germinans data of Kibler et al., 2022
                                if isinstance(dislodgethresh, (int, float)):
                                    fdislodge = dislodgethresh
                                if 'fwave' in locals() and fwave >= fdislodge: # If the wave force >= dislodgement threshold, mangrove is removed from the system
                                    dmgregdis[b,j] = 2 # Set damage tracker to 2 (mortality) so that we can output which mangroves are affected by dislodgement
                                    for item in vegtrklist: # Set all veg properties to zero
                                        item[b,j] = 0 
                                    vegtrkht[b,j] = 0
                                    vegtrkpnechk[b,j,:] = 0
                ...
                # Set overall damage regime tracker (v.2.0 update to indentation)
                dmgreglist[(b,j)] = [dmgregin[b,j],dmgregde[b,j],dmgregbu[b,j],dmgregse[b,j],dmgregdis[b,j],dmgregover[b,j],dmgregbrk[b,j]] # Create list of damage regimes
                ...
                # System removal # (v.2.0 update to indentation)
        ...
        if growthactive == 'Y': # Check if user has activated the growth stage
            for b in range(maxcapacity): # For every stem
                ...
                if vegtrkht[b,j] < sapht and vegtrk[b,j] == 1: # Stem height less than defined sapling height
                    ...
                    if vegtrkrt_seedling_growth == 'balke2015':
                        rtlengthrate = growthfactorrt * (0.003 * mtpervt / (24*3600)) # Update root growth for these anchored seeds (gradient of Fig. 3 in Balke et al., 2015)
                        vegtrkrt[b,j] += rtlengthrate # (v.2.0 update)
                    if isinstance(vegtrkrt_seedling_growth, (int, float)):
                        rtlengthrate = growthfactorrt * (vegtrkrt_seedling_growth * mtpervt / (24*3600))
                        vegtrkrt[b,j] += rtlengthrate # (v.2.0 update)
                    ...
                    if seedrtdia2len == 'basyuni2018': # Derived from the data presented by Basyuni et al., 2018
                        rtdiagthrate = rtlengthrate / 43 * mtpervt / (24*3600) # Calculate root diameter
                        vegtrkrtdia[b,j] += rtdiagthrate
                    if isinstance(seedrtdia2len, (int, float)):
                        rtdiagthrate = rtlengthrate / seedrtdia2len * mtpervt / (24 * 3600)
                        vegtrkrtdia[b,j] += rtdiagthrate
                    ...
                    if vegtrkrt[b,j] >= seedrtthresh: # If the root length is above threshold for stem growth
                        ...
                        if vegtrkdiagrowth == 'hastuti2018':
                            diagthrate = growthfactor * (0.00004 * mtpervt / (24*3600)) # Calculate shoot diameter (v.2.0 update)
                            vegtrkdia[b,j] += diagthrate # (v.2.0 update)
                        if isinstance(vegtrkdiagrowth, (int, float)):
                            diagthrate = growthfactor * (vegtrkdiagrowth * mtpervt / (24*3600)) # Calculate shoot diameter (v.2.0 update) 
                            vegtrkdia[b,j] += diagthrate # (v.2.0 update)
                        ...
                        if seedht2dia == 'dunlop2025' and vegtrkdia[b,j] >= 0.025: # This is based on Equation 1 for A. marina from Dunlop et al., 2025 (v.2.0 update)
                            # Note that this equation is only valid for stem diameters > 0.025m
                            vegtrkht[b,j] += (4.53/vegtrkdia[b,j]) * diagthrate # The derivative of Equation 1 from Dunlop et al., 2025 is multiplied by the stem diameter growth rate (v.2.0 update)
                        elif vegtrkdia[b,j] < 0.025: # (v.2.0 update)
                            vegtrkht[b,j] += diagthrate * 25 # Default back to jacotot2019 for smaller mangroves outside range of Dunlop et al., 2025 equation (v.2.0 update)
                        if seedht2dia == 'trettin2016' and vegtrkdia[b,j] >= 0.05: # This is based on the height to diameter equation for A. marina from Trettin et al., 2016 (v.2.0 update)
                            # Note that this equation is only valid for stem diameters > 0.05m
                            vegtrkht[b,j] += (5.65/vegtrkdia[b,j]) * diagthrate # The derivative of the equation for A. marina is multiplied by the stem diameter growth rate (v.2.0 update)
                        elif vegtrkdia[b,j] < 0.025: # (v.2.0 update)
                            vegtrkht[b,j] += diagthrate * 25 # Default back to jacotot2019 for smaller mangroves outside range of Trettin et al., 2016 equation (v.2.0 update)
                        if seedht2dia == 'jacotot2019': # This relationship is based on the basal diameter and stem height growth for A. marina under natural tidal flooding in New Caledonia (Jacotot et al., 2019)
                            vegtrkht[b,j] += diagthrate * 25 # Seedling height in the order of 25 x diameter (v.2.0 update)
                        if isinstance(seedht2dia, (int, float)): # If the user has defined a single numerical value instead of an equation
                            vegtrkht[b,j] += diagthrate * seedht2dia # (v.2.0 update)
                        ...
                        for p in range(maxpneums):
                            if vegtrkpnechk[b,j,p] == 1 and maxwd[j] >= pnelenreq * (vegtrkpnelen[b,j,p] - cablertdepth - (blh[j] - vegtrkbed[b,j])): # If a pneum exists and the max water depth is >= the height of the pneum (since they grow to reach oxygen)
                                ...
                                # Calculate pneumatophore height/length growth (v.2.0 update)
                                if agpnelen[b,j,p] < pnelimit - blh[j]: # If the aboveground pneumatophore length is less than the distance between the pneum ceiling and the bed level, continue growth
                                ...
                                # Continue growth if inundation occurs (v.2.0 update)
                                elif c_instr[b,j] > 1: # i.e., if the counter for inundation is 2 or greater, that is flooding has occurred for more than 1 timestep, continue growth
                                    # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for pneumatophore height growth
                                    if pnelen2pnedia == "jereznova2022":
                                        vegtrkpnelen[b,j,p] += growthfactorrt * (10.3 * vegtrkpnedia_adult_growth * mtpervt / (24*3600)) # metres/day
                                    if isinstance(pnelen2pnedia, (int, float)):
                ...
                if vegtrkht[b,j] >= sapht: # Stems that have grown beyond the seedling stage 
                    ...
                    if vegtrkdia[b,j] < maxstemdia:
                        if vegtrkdia_adult_growth == 'rajkumar2017':
                            adult_diagthrate = growthfactor * (0.00003 * mtpervt / (24*3600)) # (v.2.0 update)
                            vegtrkdia[b,j] += adult_diagthrate # (v.2.0 update)
                        if isinstance(vegtrkdia_adult_growth, (int, float)):
                            adult_diagthrate = growthfactor * (vegtrkdia_adult_growth * mtpervt / (24*3600)) # (v.2.0 update)
                            vegtrkdia[b,j] += adult_diagthrate
                    # Calculate change in stem height
                    # NOTE: OPTIONAL USER UPDATE - additional if statements can be included below for stem height to diameter relationship
                    if vegtrkht[b,j] < maxstemht:
                        if dbh2ht == 'dunlop2025' and vegtrkdia[b,j] >= 0.025: # This is based on Equation 1 for A. marina from Dunlop et al., 2025 (v.2.0 update)
                            vegtrkht[b,j] += (4.53/vegtrkdia[b,j]) * adult_diagthrate # The derivative of Equation 1 from Dunlop et al., 2025 is multiplied by the stem diameter growth rate (v.2.0 update)
                        elif dbh2ht == 'dunlop2025' and vegtrkdia[b,j] < 0.025: # (v.2.0 update)
                            vegtrkht[b,j] += adult_diagthrate * 25 # Default back to jacotot2019 for smaller mangroves outside range of Dunlop et al., 2025 equation (v.2.0 update)
                        if dbh2ht == 'trettin2016' and vegtrkdia[b,j] >= 0.05: # This is based on the height to diameter equation for A. marina from Trettin et al., 2016 (v.2.0 update)
                            vegtrkht[b,j] += (5.65/vegtrkdia[b,j]) * adult_diagthrate # The derivative of the equation for A. marina is multiplied by the stem diameter growth rate (v.2.0 update)
                        elif dbh2ht == 'trettin2016' and vegtrkdia[b,j] < 0.05: # (v.2.0 update)
                            vegtrkht[b,j] += adult_diagthrate * 25 # Default back to jacotot2019 for smaller mangroves outside range of Dunlop et al., 2025 equation (v.2.0 update)
                        if dbh2ht == 'thampanya2006': # This is based on resolving simultaneous equations in Thampanya 2006 dissertation for A. marina
                            vegtrkht[b,j] += 0.9/1.45 * adult_diagthrate # (v.2.0 update)
                        if isinstance(dbh2ht, (int, float)): # If user defines a constant value as the relationship between DBH and height
                            vegtrkht[b,j] += dbh2ht * adult_diagthrate # (v.2.0 update)
                    ...
                    if vegtrkrt[b,j] < maxrtlen:
                        ...
                        if isinstance(vegtrkrt_adult_growth, (int, float)):
                            rtlenadultgthrate = growthfactorrt * (vegtrkrt_adult_growth * mtpervt / (24*3600)) # (v.2.0 update)
                            vegtrkrt[b,j] += rtlenadultgthrate # (v.2.0 update)
                    ...
                    if vegtrkrtdia[b,j] < maxrtdia:
                        ...
                        if vegtrkrtdia_adult_growth == 'allometric1':
                            rtdiaadultgthrate = rtlenadultgthrate / 200 # Assumed growth rate to be 1/200 of the cable root length growth (v.2.0 update)
                            vegtrkrtdia[b,j] += rtdiaadultgthrate # (v.2.0 update)
    ...
    # ======================================================================
    # PREPARE VARIABLES FOR FUNCTIONALITY AND PLOTTING (v.2.0 update rearrangement)
    # ======================================================================
    # Calculate the non-zero percentiles of stem and pneumatophore heights and diameters #
    # Calculate percentile for each unique value along the second dimension

    # Stem height and bed level
    hv_stem[:] = 0 # Reset hv_stem before updating (v.2.0 update)
    ...
    for idx in unique_indices:
        ...
        # Calculate percentile for non-zero values
        if len(non_zero_hvsvalues) == 0: # (v.2.0 update)
            percentile_value = 0 # Explicitly set to zero if no values (v.2.0 update)
        else: # (v.2.0 update)
            percentile_value = np.percentile(non_zero_hvsvalues, percentileveg) # (v.2.0 update)
        # Update corresponding entry in hv_stem
        hv_stem[idx] = percentile_value
        vegbed[idx] = percentile_bed

    # Stem diameter
    stemdia[:] = 0 # Reset stemdia before updating (v.2.0 update)
    ...
    for p in range(ndx): # For every grid cell
        vegcheck[p] = check_column(stemtrk,p) # Assign a value of 1 if there is at least one stem in the grid cell (v.2.0 update)

    # Above-ground pneumatophore height/length
    hv_pneum[:] = 0 # Reset hv_pneum before updating (v.2.0 update)
    ...
    pneumdia[:] = 0 # Reset pneumdia before updating (v.2.0 update)
    ...
    if funcactive == 'Y': # Check if user activated the functionality stage
        ...
        if dragcoeff == 'vanmaanen2015':
            ...
            am_positive = am > 0 # Remove accidental negative values and only include those that are positive (v.2.0 update)
            L[am_positive] = (vcontrol[am_positive] - vm[am_positive])/am[am_positive] # Updated to am_positive (v.2.0 update)
            cdvegsp_updated[am_positive] = cd_no + 5 / L[am_positive] # Updated to am_positive (v.2.0 update)
            ...
            for b in range(maxcapacity):
                ...
                if agb == 'komiyama2005':
                    if vegtrkdia[b,j] > 0.05 and vegtrkdia[b,j] < 0.49: # Komiyama 2005 common equation based on sample trees with diameters between 0.05m and 0.49m
                        agb_stem[b,j] = 0.251 * meanwooddensity / 1000 * (vegtrkdia[b,j] * 100) ** 2.46 # kg. Sum of above-ground biomass for stems
    ...
    with np.errstate(divide='ignore', invalid='ignore'): # To ignore inf when dividing by 0 if all values are 0
        dmgregingc = np.sum(dmgregin, axis=0) / np.count_nonzero(dmgregin, axis=0)
        dmgregingc = np.nan_to_num(dmgregingc) # Replace NaNs or infs with 0
        dmgregdegc = np.sum(dmgregde, axis=0) / np.count_nonzero(dmgregde, axis=0)
        dmgregdegc = np.nan_to_num(dmgregdegc) # Replace NaNs or infs with 0
        dmgregbugc = np.sum(dmgregbu, axis=0) / np.count_nonzero(dmgregbu, axis=0)
        dmgregbugc = np.nan_to_num(dmgregbugc) # Replace NaNs or infs with 0
        dmgregsegc = np.sum(dmgregse, axis=0) / np.count_nonzero(dmgregse, axis=0)
        dmgregsegc = np.nan_to_num(dmgregsegc) # Replace NaNs or infs with 0
        dmgregdisgc = np.sum(dmgregdis, axis=0) / np.count_nonzero(dmgregdis, axis=0)
        dmgregdisgc = np.nan_to_num(dmgregdisgc) # Replace NaNs or infs with 0
        dmgregovergc = np.sum(dmgregover, axis=0) / np.count_nonzero(dmgregover, axis=0)
        dmgregovergc = np.nan_to_num(dmgregovergc) # Replace NaNs or infs with 0
        dmgregbrkgc = np.sum(dmgregbrk, axis=0) / np.count_nonzero(dmgregbrk, axis=0)
        dmgregbrkgc = np.nan_to_num(dmgregbrkgc) # Replace NaNs or infs with 0
        dmgregallgc = np.sum(dmgregall, axis=0) / np.count_nonzero(dmgregall, axis=0)
        dmgregallgc = np.nan_to_num(dmgregallgc) # Replace NaNs or infs with 0
    blgc = blh # Set plotting variable at each grid cell to be equal to bed level at the current timestep i.   

    # Reset other damage regime trackers (v.2.0 update)
    for j in range(ndx):
        for b in range(maxcapacity):
            if vegtrk[b,j] == 0 or stemtrk[b,j] == 0: # (v.2.0 update)
                dmgreglist[(b,j)] = [0]*7 # Reset the list of all damage regimes to 0 for mangroves that have been removed from the system (v.2.0 update)
                # Reset individual damage regimes
                dmgregin[b,j] = 0
                dmgregde[b,j] = 0
                dmgregbu[b,j] = 0
                dmgregse[b,j] = 0
                ...
                dmgregbrk[b,j] = 0
                # Reset trackers
                c_instr[b,j] = 0 # Reset inundation tracker
                c_destr[b,j] = 0 # Reset desiccation counter
                c_bustr[b,j] = 0 # Reset burial counter
                
    # ======================================================================
    # PLOTTING
    # ======================================================================
    # Plotting key variables at the user-specified intervals
    if i % plotint == 0 or i == nt-1 or i in range(359, 368): # Check whether the timestep corresponds to the defined interval or the last timestep in the model run
        ...
        indstemvar = {
            ...
            "indstemtrk": stemtrk,
            "indstemht": vegtrkht,
            "indstemdia": vegtrkdia,
            "indstemage": vegtrkage,
            "indstemesttime": vegtrktime,
            "indcablertdia": vegtrkrtdia,
            "indcablertlen": vegtrkrt,
            "indinund": dmgregin,
            "inddesicc": dmgregde,
            "indburial": dmgregbu,
            "inddisl": dmgregdis,
        ...
        # Loop through and save each variable as a CSV
        for name, data in indstemvar.items():
            header = [str(i+1) for i in range(ndx)] # Create header
            df = pandas.DataFrame(data, columns=header)  # Convert to DataFrame
            df.to_csv(os.path.join(csvfolder_path,f'{today}_{i}_{name}.csv'),index=False)  # Save CSV with the variable name
        ...
        for idx, y_coord_value in enumerate(xsecycoords):
            ...
            plt.ylim(-1, 15) # To be updated based on expected mangrove heights and bed levels
        ...
        if hwav is not None:
            variables = [vegcheck, hv_stem, stemdia, rnveg, ages, hv_pneum, pneumdia, n_pne, \
                        dmgregingc, dmgregdegc, dmgregbugc, dmgregsegc, dmgregdisgc, dmgregovergc, dmgregbrkgc, dmgregallgc, \
                        vm, am, cdvegsp, blgc, agb_total, bgb_total, maxwd, minwd, maxbss, hwav] # Included wave height (v.2.0 update)
            variable_names = ['Mangrove presence (-)', 'Stem height (m)', 'Stem diameter (m)', 'Stem density (stems per m2)', 'Stem age (months)', 'Above ground pneumatophore height (m)', 'Pneumatophore diameter (m)', \
                            ...
                            'Seedling dislodgement mortality (-)', 'Overturning failure (-)', 'Breaking failure (-)', \
                                'Mortality and recovery (-)', 'Projected volume (m3)', 'Projected area (m2)', 'Drag coefficient (-)', 'Bed level (m)', 'Above-ground biomass (kg)', 'Below-ground biomass (kg)', \
                                    'Maximum water depth (m)', 'Minimum water depth (m)', 'Maximum bed shear stress (Nm-2)', 'RMS wave height (m)'] # Included wave height (v.2.0 update)
        else: # Remove hwav for non wave models (v.2.0 update)
            variables = [vegcheck, hv_stem, stemdia, rnveg, ages, hv_pneum, pneumdia, n_pne, \
                        dmgregingc, dmgregdegc, dmgregbugc, dmgregsegc, dmgregdisgc, dmgregovergc, dmgregbrkgc, dmgregallgc, \
        ...
        for u, variable in enumerate(variables):
            ...
            cmap5 = plt.get_cmap('Spectral') # Red to purple colour map for the max bed shear stress plot
            norm5 = Normalize(vmin=np.min(variable), vmax=np.max(variable))

            if u < 8 or (u > 15 and u < 22): # For plots that aren't damage regimes, set these to have a yellow to green gradient
            ...
            elif u >= 8 and u <= 15: # For damage regime plots, set these to have a green to yellow to red gradient
                # Iterate through grid cell centres and plot them with colour coding
                for x_center, y_center, data in zip(x_coord, y_coord, variable):
                    # Define the coordinates of the rectangle (grid cell)
                    x_left = x_center - 2.5
            ...
            elif u == 22 or u == 23: # For max water depth plot
                # Iterate through grid cell centres and plot them with colour coding
                for x_center, y_center, data in zip(x_coord, y_coord, variable):
                    # Define the coordinates of the rectangle (grid cell)
                    x_left = x_center - 2.5
            ...
            elif u == 24: # For max bed shear stress plot
                # Iterate through grid cell centres and plot them with colour coding
                for x_center, y_center, data in zip(x_coord, y_coord, variable):
                    # Define the coordinates of the rectangle (grid cell)
                    x_left = x_center - 2.5
            ...
            elif u == 25: # For hwav plot (v.2.0 update)
                # Iterate through grid cell centres and plot them with colour coding
                for x_center, y_center, data in zip(x_coord, y_coord, variable):
                    # Define the coordinates of the rectangle (grid cell)
                    x_left = x_center - 2.5
                    ...
                    y_bottom = y_center - 2.5
                    y_top = y_center + 2.5
                    color = cmap5(norm5(data))
                    # Plot the rectangle with colour based on stem height
                    rect = plt.Rectangle((x_left, y_bottom), x_right - x_left, y_top - y_bottom, color=color, edgecolor='black')
                    ax.add_patch(rect)
                # Add colour bar
                sm = plt.cm.ScalarMappable(cmap=cmap5, norm=norm5)
            ...
            x_values_to_show = [0, 50, 100, 150, 200, 250, 300, 350]
    ...
    # Reset damage regime trackers (v.2.0 update)
    # These trackers are reset after the plotting / exporting data stage because it is assumed that the mangroves are removed completely and the values have been added to the plotting variables above
    dmgregdis = np.zeros((maxcapacity, ndx))
    dmgregover = np.zeros((maxcapacity, ndx))
    dmgregbrk = np.zeros((maxcapacity, ndx))
